...
def generation_prompt(request: GenerationRequest) -> str:
    return (
        ...
        "'meaning', and 'example'. The front must be a single natural English "
        "sentence that contains the target term highlighted exactly once using "
        "<b>...</b>; never return the target term alone without a sentence. Put "
        "the target term followed by ': ' and its "
